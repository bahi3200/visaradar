...
const socialIcons: Record<string, { icon: React.ReactNode; label: string; fallback: string }> = {
  public_facebook_url: {
    icon: (
...
export default function Footer() {
  ...
  // نعرض دائماً الأيقونات الأربع، مع تفضيل رابط الأدمن إن توفر.
  const activeSocials = Object.entries(socialIcons).map(([key, cfg]) => {
        ...
        <div className="flex items-center gap-4">
          {activeSocials.map(([key, { icon, label, url }]) => (
            <a
              ...
              aria-label={label}
              className="w-9 h-9 rounded-xl bg-secondary/50 border border-border/50 flex items-center justify-center text-muted-foreground hover:text-primary hover:border-primary/40 hover:bg-primary/10 transition-all"
            >
              {icon}
            </a>
          ))}
