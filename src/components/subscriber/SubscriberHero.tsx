...
export default function SubscriberHero({ fullName, packageName, daysLeft, expiresAt, isSubscribed, isAdmin }: Props) {
  ...
  const badgeClass = isAdmin ? "bg-primary/20" : isSubscribed ? "gradient-accent" : "bg-primary/20";
  const badgeTextClass = isAdmin ? "text-primary" : isSubscribed ? "text-accent" : "text-primary";
        ...
        <div className="container pb-8">
          ...
          >
            <div className="flex items-center gap-2 mb-2">
              <div className={`w-8 h-8 rounded-lg ${badgeClass} flex items-center justify-center`}>
                <BadgeIcon className={`w-4 h-4 ${isSubscribed && !isAdmin ? 'text-accent-foreground' : badgeTextClass}`} />
              </div>
              <span className={`text-xs font-bold ${badgeTextClass}`}>{badgeLabel}</span>
            </div>
            ...
            <p className="text-sm text-muted-foreground">
              {isAdmin
                ...
                : "مرحباً بك في منصتنا — اشترك للاستفادة من جميع المزايا"}
            </p>
          </motion.div>
