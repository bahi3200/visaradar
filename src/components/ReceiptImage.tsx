  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (target && (target.tagName === "INPUT" || target.tagName === "TEXTAREA" || target.isContentEditable)) {
        console.log("[ReceiptLightbox] ignored: input/textarea focused");
        return;
      }
      const t = transformRef.current;
      if ((e.key === "+" || e.key === "=") && t) { e.preventDefault(); console.log("[ReceiptLightbox] zoomIn"); t.zoomIn(); }
      else if ((e.key === "-" || e.key === "_") && t) { e.preventDefault(); console.log("[ReceiptLightbox] zoomOut"); t.zoomOut(); }
      else if (e.key === "0" && t) { e.preventDefault(); console.log("[ReceiptLightbox] reset"); t.resetTransform(); }
      else if (e.key === "f" || e.key === "F") { e.preventDefault(); console.log("[ReceiptLightbox] fullscreen toggle"); toggleFullscreen(); }
